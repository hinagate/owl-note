...
describe('ask-panel — tidy quick action (E11)', () => {
  it('renders a Tidy button beside Summarize, sharing the chip lifecycle', () => {
    ...
    expect(btn).not.toBeNull();
    expect(btn.textContent).toBe('Tidy');
    expect(btn.getAttribute('aria-label')).toBe('Tidy the note formatting');
    expect(el.querySelector('.ask-summarize')).not.toBeNull(); // sits beside Summarize

    el.querySelector('.ask-chip-dismiss').click();
    expect(el.querySelector('.ask-chip')).toBeNull();
    expect(el.querySelector('.ask-tidy')).toBeNull(); // gone with the chip (no orphan focusable)
  });
